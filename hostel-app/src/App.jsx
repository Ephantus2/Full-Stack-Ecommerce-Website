...
function App() {
  ...
  const addToCart = (product) => {
    setCart(c => {
      const existingItem = c.find(item => item.productName == product.description);
      if(existingItem){
        return c.map(item => {
          return item.productName == product.description ? {...item, quantity: item.quantity + 1} : item;
        })
      }else{
        return [...c, {productName: product.description,
                        image: product.image,
                        price: product.price, 
                        quantity: 1}]
      }
